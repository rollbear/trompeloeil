...
Usage:
  trompeloeil_class.py header-file.h [ClassName]...
  ...
  for node in class_node.body:
    ...
    if (isinstance(node, ast.Function) and node.modifiers & function_type and
      ...
      if class_node.templated_types:
        tmpl = '_T'
      mock_method_macro = 'MAKE_%sMOCK%d%s' % (const, num_parameters, tmpl)
...
def _GenerateMocks(filename, source, ast_list, desired_class_names):
  ...
  lines = []
  lines.extend(['#include <trompeloeil.hpp>'])
  lines.append('')
  # strip filename to actual header file
  lines.extend(['#include \"%s\"' % filename.split('/')[-1] ])
  lines.append('')
  # Now for the actual mock class
